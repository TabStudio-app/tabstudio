...
import { buildVideoExportBlob, collectVideoSyncNoteSequence, formatTapSyncTimestamp } from "./features/export/exportHelpers";
...
export default function App() {
  ...
  if (routePath === "/" || routePath === "/editor" || routePath === "/projects" || routePath === "/export") {
    return (
      <EditorApp
        navigateTo={navigateTo}
        pendingOpenPanel={routePath === "/projects" ? "projects" : routePath === "/export" ? "export" : pendingEditorPanel}
...
function EditorApp({ navigateTo, pendingOpenPanel = "", onPendingPanelHandled, updateUserState, userState }) {
  ...
  const accountEmail = "support@tabstudio.app";
...
function fillSelectedColumnWith(value) {
  ...
  function saveChordExportTuning(name, lowToHigh) {
    ...
    const notes = Array.isArray(lowToHigh) ? lowToHigh.map((s) => String(s ?? "").trim()).slice(0, 6) : [];
    if (!trimmedName) return null;
    if (notes.length !== 6 || notes.some((s) => !s)) return null;
    const id = `user_tuning_${Date.now()}_${Math.random().toString(16).slice(2)}`;
  ...
  function saveCustomChordToLibrary(name, frets, { selectSavedChord = true } = {}) {
    if (!chordToolEnabled) return "";
    const trimmedName = String(name || "").trim();
    if (!trimmedName) return "";
    const nextFrets = Array.isArray(frets) ? frets.map((value) => String(value ?? "").trim()).slice(0, 6) : [];
    while (nextFrets.length < 6) nextFrets.push("");
    ...
    setUserChords((prev) => [
      ...prev,
      {
        ...
        name: trimmedName,
        frets: nextFrets,
        tuningId: currentChordTuningId,
        tuningName: String(tuningLabel || "Custom"),
        instrumentId: String(currentInstrument.id || "gtr6"),
      },
    ]);
  ...
  const exportBrandingText = useMemo(
    ...
    [imageShowBranding, exportUseAffiliateBranding, userState?.profile]
  );
  ...
  useEffect(() => {
    ...
    const nextInclude = profilePrefs.includeTabStudioLink !== false;
  ...
  }, [userState?.profile?.exportBrandingPrefs]);
  ...
  useEffect(() => {
    updateUserState?.((prev) => {
      ...
      const nextPrefs = {
        includeTabStudioLink: imageShowBranding,
  ...
  }, [imageShowBranding, exportUseAffiliateBranding, exportAffiliateLinkText, updateUserState]);
  ...
  async function exportVideoNow() {
    ...
    try {
      ...
      const includeBranding = videoBrandingMode === "tabstudio" || videoBrandingMode === "affiliate";
      ...
      >
        ...
        <div style={siteHeaderRightGroupStyle}>
          ...
          <div ref={saveProjectsGroupRef} style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            {!accountProfileOpen && !projectsLibraryOpen && !exportModalOpen && editorSaveStatus ? (
              <div
                style={{
                  fontSize: 12,
                  fontWeight: 800,
                  color:
                    editorSaveStatus === "All changes saved"
                      ? withAlpha(THEME.text, 0.7)
                      : editorSaveStatus === "Saving..."
                        ? withAlpha(THEME.accent, 0.84)
