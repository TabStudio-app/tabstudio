...
const DEFAULT_SLOGAN_OFFSET_X = -50;
...
function countTabsInLibraryData(data) {
  ...
  for (const artistName of Object.keys(artists)) {
    ...
    for (const albumName of Object.keys(albums)) {
      ...
      total += Object.keys(songs).length;
    }
  }
...
const editingCellRef = useRef(null); // tracks the cell for the current typing session
  ...
  function renameLibraryArtist(artistName) {
    ...
    const nextName = String(window.prompt("Rename artist", artistName) || "").trim();
  ...
  function deleteLibraryArtist(artistName) {
    if (!artistName || artistName === "Unsorted") return;
    const ok = window.confirm(`Delete artist "${artistName}" and all albums/songs?`);
    if (!ok) return;
    setLibraryData((prev) => {
      const next = normalizeLibraryData(cloneJson(prev, makeEmptyLibrary()));
      delete next?.artists?.[artistName];
  ...
  function renameLibraryAlbum(artistName, albumNameCurrent) {
    ...
    const nextName = String(window.prompt("Rename album", albumNameCurrent) || "").trim();
  ...
  function deleteLibraryAlbum(artistName, album) {
    ...
    const ok = window.confirm(`Delete album "${album}" and all songs in it?`);
    if (!ok) return;
    setLibraryData((prev) => {
      const next = normalizeLibraryData(cloneJson(prev, makeEmptyLibrary()));
      const albums =
  ...
  function renameLibrarySong(artistName, album, song) {
    if (!artistName || !album || !song) return;
    const nextName = String(window.prompt("Rename song", song) || "").trim();
...
function fillSelectedColumnWith(value) {
  ...
  const [showSloganOffsetAdjuster, setShowSloganOffsetAdjuster] = useState(true);
  ...
  const introHasSuffix = headerIntroText.startsWith(",");
  const introPrefixText = introHasSuffix ? "" : headerIntroText;
  const introSuffixText = introHasSuffix ? headerIntroText : "";
  ...
  useEffect(() => {
    ...
    const introWords = ["Write", "Save", "Play", ", simplified"];
    const fadeInMs = 550;
    ...
    introWords.forEach((word, index) => {
      const startMs = index * totalPerWordMs;
      timers.push(
        setTimeout(() => {
    ...
    const finalWordEndMs = (introWords.length - 1) * totalPerWordMs + fadeInMs + visibleMs;
    timers.push(
      setTimeout(() => {
        setHeaderSloganReady(true);
        timers.push(setTimeout(() => setHeaderIntroOpacity(0), 90));
        timers.push(
          setTimeout(() => {
            setHeaderIntroVisible(false);
            setHeaderIntroText("");
        ...
        timers.push(setTimeout(() => setHeaderSloganTextOpacity(1), 120));
        timers.push(setTimeout(() => setHeaderSloganAtFinalX(true), 460));

        setHeaderSloganTickVisible(true);
        setHeaderSloganTickOpacity(1);
        timers.push(setTimeout(() => setHeaderSloganTickOpacity(0), 760));
        timers.push(setTimeout(() => setHeaderSloganTickVisible(false), 1420));
        try {
  ...
  const availableArtistNames = useMemo(() => Object.keys(libraryData?.artists || {}).sort(), [libraryData]);
  const effectiveArtistLabel = String(artist || "").trim() || "Unsorted";
  const albumsForCurrentArtist = useMemo(() => {
    if (!effectiveArtistLabel || effectiveArtistLabel === "Unsorted") {
      return Object.keys(libraryData?.unsorted?.albums || {}).sort();
    }
    return Object.keys(libraryData?.artists?.[effectiveArtistLabel]?.albums || {}).sort();
  }, [libraryData, effectiveArtistLabel]);
  const selectedLibraryArtistLabel = selectedLibraryArtistKey || "Unsorted";
  const selectedLibraryAlbums = useMemo(() => {
    if (selectedLibraryArtistLabel === "Unsorted") {
      return Object.keys(libraryData?.unsorted?.albums || {}).sort();
    }
    return Object.keys(libraryData?.artists?.[selectedLibraryArtistLabel]?.albums || {}).sort();
  }, [libraryData, selectedLibraryArtistLabel]);
  const selectedLibrarySongs = useMemo(() => {
    if (!selectedLibraryAlbumName) return [];
    const songs =
      selectedLibraryArtistLabel === "Unsorted"
        ? libraryData?.unsorted?.albums?.[selectedLibraryAlbumName]?.songs || {}
        : libraryData?.artists?.[selectedLibraryArtistLabel]?.albums?.[selectedLibraryAlbumName]?.songs || {};
    return Object.keys(songs).sort((a, b) => a.localeCompare(b));
  }, [libraryData, selectedLibraryArtistLabel, selectedLibraryAlbumName]);
  ...
  useEffect(() => {
    if (!uiDialog || uiDialog.type !== "prompt") return;
    requestAnimationFrame(() => {
      try {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {showHeaderSlogan && (
            <div
              style={{
                marginLeft: headerSloganAtFinalX ? sloganOffsetX : sloganOffsetX + SLOGAN_INTRO_OFFSET_DELTA,
                width: 220,
                minWidth: 220,
                display: "inline-flex",
                alignItems: "center",
                ...
                transition: "margin-left 1400ms cubic-bezier(0.16, 1, 0.3, 1)",
              }}
            >
              {headerSloganReady ? (
                <>
                ...
                <>
                  ...
                  >
                    {headerIntroVisible ? introPrefixText : ""}
                  </span>
                  <span style={{ width: 28, minWidth: 28, textAlign: "center", opacity: isDarkMode ? 0.8 : 0.72 }}>Tabs</span>
                  <span
                    style={{
                      display: "inline-block",
                      width: 88,
                  ...
                  >
                    {headerIntroVisible ? introSuffixText : ""}
                  </span>
                </>
              )}
            </div>
      ...
      {showHeaderSlogan && showSloganOffsetAdjuster && (
        <div
          style={{
            position: "fixed",
            left: 18,
        ...
        >
          <span style={{ fontSize: 12, color: THEME.textFaint, whiteSpace: "nowrap" }}>Slogan X</span>
          <input
            type="range"
            min="-120"
            max="40"
            step="1"
            value={sloganOffsetX}
            onChange={(e) => setSloganOffsetX(Number(e.target.value))}
            aria-label="Adjust slogan horizontal position"
          />
          <span style={{ width: 34, textAlign: "right", fontSize: 12, fontWeight: 700 }}>{sloganOffsetX}</span>
          <button
            type="button"
            onClick={() => setShowSloganOffsetAdjuster(false)}
            style={{
              border: `1px solid ${THEME.border}`,
              background: "transparent",
              color: THEME.textFaint,
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <button
                  ...
                  style={{ ...btnSecondary, height: 28, padding: "0 10px", borderRadius: 999, fontSize: 12, fontWeight: 900 }}
                  ...
                  {artistCreateOpen ? (
                    ...
                    <>
                      <select
                        id="artist-select"
                        ref={artistSelectRef}
                        value={String(artist || "").trim() || "__unsorted__"}
                        onChange={(e) => {
                          const value = String(e.target.value || "");
                          if (value === "__new_artist__") {
                            setNewArtistDraft("");
                          ...
                          setArtistCreateOpen(false);
                          setNewArtistDraft("");
                          setArtist(value === "__unsorted__" ? "" : value);
                          setAlbumName("");
                        }}
                        onKeyDown={(e) => {
                          if (e.key !== "Escape") return;
                        ...
                        style={{ ...field, fontWeight: 800, appearance: "none", WebkitAppearance: "none", paddingRight: 34 }}
                      >
                        ...
                        <option value="__new_artist__">+ New artist...</option>
                      </select>
                      <span
                        aria-hidden="true"
                        style={{
                  ...
                  {albumCreateOpen ? (
                    ...
                    <>
                      <select
                        id="album-select"
                        ref={albumSelectRef}
                        value={String(albumName || "").trim() || "__no_album__"}
                        onChange={(e) => {
                          const value = String(e.target.value || "");
                          if (value === "__new_album__") {
                            setNewAlbumDraft("");
                          ...
                          setAlbumCreateOpen(false);
                          setNewAlbumDraft("");
                          setAlbumName(value === "__no_album__" ? "" : value);
                        }}
                        onKeyDown={(e) => {
                          if (e.key !== "Escape") return;
                          e.preventDefault();
                        ...
                        style={{ ...field, fontWeight: 800, appearance: "none", WebkitAppearance: "none", paddingRight: 34 }}
                      >
                        <option value="__no_album__">{NO_ALBUM_NAME}</option>
                        {albumsForCurrentArtist
                          .filter((name) => name !== NO_ALBUM_NAME)
                          .map((name) => (
                            <option key={name} value={name}>
                              {name}
                            </option>
                          ))}
                        <option value="__new_album__">+ New album...</option>
                      </select>
                      <span
                        aria-hidden="true"
                        style={{
                        ...
                        >
                          ...
                          {isUser ? (
                            <button
                              ...
                              style={{ ...actionDeleteBtn, width: 40, minWidth: 40, height: 40, borderRadius: 12, fontSize: 20 }}
                            >
                              ×
                            </button>
                          ) : (
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              ...
                              style={{ ...actionEditBtn, height: 32, padding: "0 10px", fontSize: 11 }}
                          ...
                          return (
                            ...
                            >
                              ...
                              <button
                                ...
                                style={{ ...actionDeleteBtn, height: 32, minWidth: 32, width: 32, fontSize: 18 }}
                              >
                                ×
                              </button>
                            </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <button
                                ...
                                style={{ ...actionDeleteBtn, width: 28, minWidth: 28, height: 28, borderRadius: 8, fontSize: 16 }}
                                title="Remove item"
              ...
              >
                ...
                >
                  <div style={{ ...card, padding: 10, minHeight: 0, overflowY: "auto" }}>
                    ...
                    <div style={{ display: "grid", gap: 6 }}>
                      {["Unsorted", ...availableArtistNames].map((artistName) => {
                        const active = selectedLibraryArtistLabel === artistName;
                        return (
                          <div key={artistName} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto auto", gap: 6 }}>
                            <button
                              type="button"
                              onClick={() => {
                                setSelectedLibraryArtistKey(artistName === "Unsorted" ? "" : artistName);
                            ...
                            {artistName !== "Unsorted" && (
                              <>
                                <button
                                  type="button"
                                  onClick={() => renameLibraryArtist(artistName)}
                                  style={{ ...actionEditBtn, height: 36, padding: "0 10px", fontSize: 11 }}
                                >
                                  Edit
                                </button>
                                <button
                                  type="button"
                                  onClick={() => deleteLibraryArtist(artistName)}
                                  style={{ ...actionDeleteBtn }}
                                >
                                  ×
                                </button>
                              </>
                  ...
                  <div style={{ ...card, padding: 10, minHeight: 0, overflowY: "auto" }}>
                    ...
                    <div style={{ display: "grid", gap: 6 }}>
                      {!selectedLibraryArtistLabel ? (
                        ...
                        selectedLibraryAlbums.map((album) => {
                          const active = selectedLibraryAlbumName === album;
                          return (
                            <div key={album} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto auto", gap: 6 }}>
                              <button
                                type="button"
                                onClick={() => {
                                  setSelectedLibraryAlbumName(album);
                              ...
                              <button
                                type="button"
                                onClick={() => renameLibraryAlbum(selectedLibraryArtistLabel, album)}
                                style={{ ...actionEditBtn, height: 36, padding: "0 10px", fontSize: 11 }}
                              >
                                Edit
                              </button>
                              <button
                                type="button"
                                disabled={album === NO_ALBUM_NAME}
                                onClick={() => deleteLibraryAlbum(selectedLibraryArtistLabel, album)}
                                style={{
                                  ...actionDeleteBtn,
                                  color: album === NO_ALBUM_NAME ? THEME.textFaint : THEME.danger,
                                  borderColor: album === NO_ALBUM_NAME ? THEME.border : withAlpha(THEME.danger, 0.35),
                                  opacity: album === NO_ALBUM_NAME ? 0.6 : 1,
                                }}
                              >
                                ×
                              </button>
                  ...
                  <div style={{ ...card, padding: 10, minHeight: 0, overflowY: "auto" }}>
                    ...
                    <div style={{ display: "grid", gap: 6 }}>
                      {!selectedLibraryAlbumName ? (
                        ...
                        selectedLibrarySongs.map((song) => {
                          const active = selectedLibrarySongName === song;
                          return (
                            <div key={song}>
                              <button
                                type="button"
                                onClick={() =>
                                  loadLibrarySongByPath(selectedLibraryArtistLabel, selectedLibraryAlbumName, song)
                                }
                                style={{
                                  ...btnSecondary,
                                  width: "100%",
                              ...
                              <div style={{ marginTop: 6, display: "flex", justifyContent: "flex-end", gap: 6 }}>
                                ...
                                <button
                                  type="button"
                                  onClick={() => renameLibrarySong(selectedLibraryArtistLabel, selectedLibraryAlbumName, song)}
                                  style={{
                                    ...actionEditBtn,
                                    height: 28,
                                    padding: "0 9px",
                                ...
                                <button
                                  type="button"
                                  onClick={() => deleteLibrarySong(selectedLibraryArtistLabel, selectedLibraryAlbumName, song)}
                                  style={{
                                    ...actionDeleteBtn,
                                    height: 28,
                                    width: 28,
                ...
                >
                  Songs are stored in localStorage using Artist - Album - Song hierarchy for this V1 build.
                </div>
